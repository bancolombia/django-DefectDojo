...
from dojo.engine_tools.models import (
    ...
    FindingExclusionStatusHistorical
)
...
class TestFindingExclusion(DojoTestCase):
    ...
    def test_finding_exclusion_discussion(self):
        ...
        self.assertEqual(
            discussion.content,
            'This is a test discussion about the finding exclusion'
        )
...
class FindingExclusionViewsTestCase(DojoTestCase):
    ...
    def test_create_finding_exclusion_view(self):
        ...
        response = self.client.post(
            reverse('create_finding_exclusion'), data=new_exclusion_data)
    ...
    def test_show_finding_exclusion_view(self):
        ...
        response = self.client.get(
            reverse('finding_exclusion', args=[str(self.exclusion1.uuid)]))

        self.assertEqual(response.status_code, 200)

        self.assertIn('finding_exclusion', response.context)
        self.assertEqual(
            response.context['finding_exclusion'], self.exclusion1)
    ...
    def test_add_finding_exclusion_discussion(self):
        ...
        response = self.client.post(
            reverse('add_finding_exclusion_discussion',
                    args=[str(self.exclusion1.uuid)]),
            data=discussion_data
        )

        self.assertRedirects(response, reverse(
            'finding_exclusion', args=[str(self.exclusion1.uuid)]))
    ...
    def test_unauthorized_access(self):
        ...
        for view_name, args in views_to_test:
            ...
            response = client_not_logged.get(
                reverse(view_name, args=view_args))
    ...
    def test_accept_finding_exclusion_request(self):
        ...
        response = self.client.post(
            reverse('accept_finding_exclusion_request',
                    args=[str(self.exclusion1.uuid)])
        )
        ...
        updated_exclusion = FindingExclusion.objects.get(
            uuid=self.exclusion1.uuid)

        self.assertEqual(updated_exclusion.status, 'Accepted')
        self.assertEqual(updated_exclusion.final_status, 'Accepted')

        findings = Finding.objects.filter(
            cve=self.exclusion1.unique_id_from_tool)
    ...
    def test_reject_finding_exclusion_request(self):
        ...
        response = self.client.post(
            reverse('reject_finding_exclusion_request',
                    args=[str(self.exclusion1.uuid)])
        )
    ...
    def test_delete_finding_exclusion_discussion(self):
        ...
        self.assertRedirects(response, reverse(
            'finding_exclusion', args=[str(self.exclusion1.uuid,)]))
        self.assertFalse(FindingExclusionDiscussion.objects.filter(
            id=discussion.id).exists())


class TestFindingExclusionStatusHistorical(DojoTestCase):
    def setUp(self):
        self.user = Dojo_User.objects.create_user(username='testuser')
        ...
        self.exclusion1 = FindingExclusion.objects.create(
            unique_id_from_tool='CVE-2023-1234',
            created_by=self.user,
            status_updated_by=self.user,
            status='Pending',
            reason='False positive',
            expiration_date=timezone.now() + timedelta(days=30)
        )

        self.exclusion1.status = "Accepted"
        self.exclusion1.save()

    def test_status_historical_values(self):
        self.assertEqual(
            FindingExclusionStatusHistorical.objects.count(), 1
        )

        self.assertEqual(
            FindingExclusionStatusHistorical.objects.all().first().last_status,
            "Pending"
        )

        self.assertEqual(
            FindingExclusionStatusHistorical.objects.all().first().new_status,
            "Accepted"
        )

        self.assertEqual(
            FindingExclusionStatusHistorical.objects.all().first().user,
            self.user
        )

    def test_html_output(self):
        record = FindingExclusionStatusHistorical.objects.all().first()
        html_output = record.to_html_log()

        self.assertIn("Status changed from", html_output)
        self.assertIn("Pending", html_output)
        self.assertIn("Accepted", html_output)
        self.assertIn("#1B30DE", html_output)
        self.assertIn("#096C11", html_output)
