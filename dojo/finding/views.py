...
class ListFindings(View, BaseListFindings):
    ...
    def get(self, request: HttpRequest, product_id: int | None = None, engagement_id: int | None = None):
        ...
        filtered_findings_qs = filtered_findings.qs
        if component_id:
            filtered_findings_qs = filtered_findings_qs.filter(component__id=component_id)
        ...
        if settings.ENABLE_FILTER_FOR_TAG_RED_TEAM:
            filtered_findings_qs = exclude_test_or_finding_with_tag(
                filtered_findings_qs,
                user=request.user)

        paged_findings = get_page_items(request, filtered_findings_qs, 25)
